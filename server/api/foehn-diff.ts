...
/** Gleiche Zeitzone wie /api/foehn-series (Vergleichbarkeit Modell ↔ Diagramm) */
...
export default defineEventHandler(async () => {
    ...
    const [bolzanoRes, omBozen, omInnsbruck, omBozenFc1, omInnFc1] = await Promise.all([
        ...
        $fetch<OpenMeteoCurrentResponse>(omBozenUrl).catch(
            () => ({}) as OpenMeteoCurrentResponse
        ),
        $fetch<OpenMeteoCurrentResponse>(omInnsbruckUrl).catch(
            () => ({}) as OpenMeteoCurrentResponse
        ),
        $fetch<HourlyPressure>(omBozenFc1Url).catch(() => ({}) as HourlyPressure),
        $fetch<HourlyPressure>(omInnFc1Url).catch(() => ({}) as HourlyPressure),
    ])

    const rows = Array.isArray(bolzanoRes?.rows) ? bolzanoRes.rows : []
    const bolzanoStation =
        rows.find((row) => matchesBozenStation(row)) ?? null
    ...
    const omBozenValid =
        omBozenMsl != null && Number.isFinite(Number(omBozenMsl))
    ...
    const differenceModel =
        firstKey != null ? (fcMap.get(firstKey) ?? null) : null

    return {
        generatedAt: new Date().toISOString(),
        bolzanoPressure,
        innsbruckPressure,
        /** Mess-/Momentanwerte (SIAG Bozen wenn möglich, sonst Modell) − Modell Innsbruck */
        differenceStation,
        /** Erste Vorhersage-Stunde, gleiche Logik wie „Vorh. Zukunft“ im Diagramm (best_match, Δp Modell) */
        differenceModel,
