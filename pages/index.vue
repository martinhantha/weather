...
import type { IWeatherData, IMeasurement } from '~/types'
...
async function update() {
    weatherData.value = await useMeasurement(<IMeasurement>data.value)
    direction.value = weatherData.value.conditions[0].wind_dir_last + 'deg'
    degrees.value = (summ / 100) * (weatherData.value.conditions[0]?.wind_speed_last ?? 0) - 135 + 'deg'
    degreesMax.value = (summ / 100) * (weatherData.value.conditions[0].wind_speed_hi_last_10_min ?? 0) - 135 + 'deg'
    directionScalar.value =
        (summ / 100) * (weatherData.value.conditions[0].wind_dir_scalar_avg_last_10_min ?? 0) - 135 + 'deg'
    counter.value++
...
<template>
    <div v-if="weatherData.conditions">
        <!-- {{ maesurement.data.json }} -->
        <div class="py-1.5">
            {{ new Date(weatherData.ts * 1000).toLocaleString() }}
        </div>
        <div class="py-1.5">
            Aktuelle Wind: <strong>{{ weatherData.conditions[0].wind_speed_last }}</strong> km/h
        </div>
        <div class="py-1.5">
            Durchschnitt Wind 10min:
            <strong>{{ weatherData.conditions[0].wind_speed_avg_last_10_min }} </strong> km/h
        </div>
        <div class="py-1.5">
            Höchstwert Wind 10min: <strong>{{ weatherData.conditions[0].wind_speed_hi_last_10_min }}</strong> km/h
        </div>
        <div class="py-1.5">
            Temperatur: <strong>{{ weatherData.conditions[0].temp }}</strong> °C
        </div>
        <div class="py-1.5">
            Feuchtikeit: <strong>{{ weatherData.conditions[0].hum }}</strong> %
        </div>
        <div class="py-1.5">
            Feuchtkugel: <strong>{{ weatherData.conditions[0].wet_bulb }}</strong> °C
        </div>
    </div>
